...
const JobListingCard = ({
  ...
  return (
    <Link href={'/oglasi-za-posao/' + href}>
      <div className="shadow-lg bg-[#FFFCE3] min-w-[100%] lg:min-w-[450px] w-full rounded-[20px] max-w-[100%] basis-[100%] p-[24px] lg:m-[20px] m-[12px]  hover:translate-y-[-10px] transition-all cursor-pointer">
        <div className='flex justify-between items-center'>
        <p className="text-xs opacity-85">Objavljeno: {formatDate(publishedAt)}</p>
        <Link href={'/oglasi-za-posao/' + href}>
        ...
        <div className="flex justify-between items-center pt-[49px]">
          <div className="flex flex-col lg:flex-row justify-between gap-3.5 lg:gap-2 items-start lg:items-center w-full text-primary">
            {<p className="m-0 text-xs">{deadline  ? 'Prijave traju do: ' + formatDate(deadline) : 'Nema roka prijave'}</p>}
            {salary && (
